...
import warnings
warnings.filterwarnings("once")  # optional: reduce duplicate warning spam
...
# ----------------------------------------------------------------------
# THE DEFINITIVE FIX: Manually build and load models to bypass 'meta' device issues
# ----------------------------------------------------------------------

print("Loading models with a manual, robust method...")

# --- Load Swin ---
swin_config = SwinConfig.from_pretrained(SWIN_ID)
swin = SwinModel(swin_config)  # Create empty model on CPU
swin_weights_path = hf_hub_download(repo_id=SWIN_ID, filename="pytorch_model.bin")
swin.load_state_dict(torch.load(swin_weights_path, map_location="cpu"))
swin = swin.to(device).eval()
print("Swin model loaded successfully.")

# --- Load DETR ---
detr_config = DetrConfig.from_pretrained(DETR_ID)
detr = DetrForObjectDetection(detr_config) # Create empty model on CPU
...
clip_vision = CLIPVisionModelWithProjection(clip_config) # Create empty model on CPU
...
if __name__ == "__main__":
    try:
        ...
        tokens, _, _ = encoder_instance(batch_paths)
        print(f"Train mode successful. Output shape: {tokens.shape}")
        
        tokens_infer, _, _ = extract_image_tokens_infer(batch_paths, encoder=encoder_instance)
        print(f"Infer mode successful. Output shape: {tokens_infer.shape}")

    except Exception as e:
        print(f"An error occurred: {e}. Please ensure you have the required libraries installed.")