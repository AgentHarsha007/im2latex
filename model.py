...
class ImageToLatexTransformer(nn.Module):
    def __init__(self,num_blocks,num_heads,vocabulary, vocab_size, pad_token_id, embed_size, dropout, forward_expansion):
        super(ImageToLatexTransformer, self).__init__()
        self.device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.pad_token_id=pad_token_id
        self.project_cnn=nn.Linear(128,embed_size)
    ...
    def forward(self,input,return_hidden=False):
        ...
        cnn_features=cnn_features+self.positional_encoding_fourier(cnn_features)
